...
const ISvcSindico = () => (
  <Svg width={28} height={28} viewBox="0 0 24 24">
    <Path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2" stroke={GOLD} strokeWidth="2" fill="none" strokeLinecap="round" />
    <Path d="M9 11a4 4 0 1 0 0-8 4 4 0 0 0 0 8z" fill={GOLD} />
    <Path d="M23 21v-2a4 4 0 0 0-3-3.87M16 3.13a4 4 0 0 1 0 7.75" stroke={GOLD} strokeWidth="2" fill="none" strokeLinecap="round" />
  </Svg>
);
...
const ISvcSeguro = () => (
  <Svg width={28} height={28} viewBox="0 0 24 24">
    <Path d="M12 2L3 7v5c0 5.25 3.75 10.15 9 11.35C17.25 22.15 21 17.25 21 12V7L12 2z" fill={GOLD} />
    <Path d="M9 12l2 2 4-4" stroke={WHITE} strokeWidth="2" fill="none" strokeLinecap="round" strokeLinejoin="round" />
  </Svg>
);
const ISvcAVCB = () => (
  <Svg width={28} height={28} viewBox="0 0 24 24">
    <Path d="M17.657 18.657A8 8 0 0 1 4.343 5.343" stroke={GOLD} strokeWidth="2" fill="none" strokeLinecap="round" />
    <Path d="M12 8v4l3 3" stroke={GOLD} strokeWidth="2" fill="none" strokeLinecap="round" strokeLinejoin="round" />
    <Path d="M12 2a10 10 0 1 0 0 20A10 10 0 0 0 12 2z" stroke={GOLD} strokeWidth="2" fill="none" />
  </Svg>
);
const ISvcJuridico = () => (
  <Svg width={28} height={28} viewBox="0 0 24 24">
    <Path d="M3 6h18M3 12h18M3 18h18" stroke={GOLD} strokeWidth="2" fill="none" strokeLinecap="round" />
    <Path d="M8 6V4a1 1 0 0 1 1-1h6a1 1 0 0 1 1 1v2" stroke={GOLD} strokeWidth="2" fill="none" strokeLinecap="round" />
  </Svg>
);
...
function PageCapa({
  ...
  return (
    <Page size="A4" style={{ flexDirection: "row", backgroundColor: WHITE }}>
      ...
      >
        <Image src={logoAlpha} style={{ width: 220, height: 110, objectFit: "contain", marginBottom: 56 }} />
        <Text style={{ fontSize: 8, color: NAVY, letterSpacing: 3.5, marginBottom: 6 }}>PROPOSTA COMERCIAL</Text>
        <Text style={{ fontSize: 20, fontWeight: "bold", color: NAVY, marginBottom: 6, letterSpacing: 0.5 }}>N {numeroContrato}</Text>
        <Text style={{ fontSize: 9, color: GRAY_500, marginBottom: 32 }}>{dataHoje}</Text>
        {nomeCondominio ? <Text style={{ fontSize: 11, fontWeight: "bold", color: NAVY, marginBottom: 4, lineHeight: 1.3 }}>{nomeCondominio}</Text> : null}
        {enderecoCondominio ? <Text style={{ fontSize: 9, color: GRAY_500, marginBottom: 4, lineHeight: 1.4 }}>{enderecoCondominio}</Text> : null}
        {numeroUnidades ? <Text style={{ fontSize: 9, color: GRAY_500, marginBottom: 8 }}>{numeroUnidades} unidades</Text> : null}
        {nomeContato ? <Text style={{ fontSize: 9, color: GRAY_500 }}>Solicitante: {nomeContato}</Text> : null}
      </View>
      ...
      >
        ...
        <View style={{ position: "absolute", bottom: 180, left: 0, right: 0, alignItems: "center" }}>
          <BuildingsSilhouette />
        </View>
        <View
          style={{
...
function PageQuemSomos({ pg, total }: { pg: number; total: number }) {
  return (
    <Page size="A4" style={{ backgroundColor: WHITE }}>
      ...
      <View style={{ flex: 1, paddingHorizontal: 60, paddingTop: 36, paddingBottom: 52, justifyContent: "center" }}>
        <Text style={{ fontSize: 8, color: GOLD, letterSpacing: 2.5, fontWeight: "bold", marginBottom: 8 }}>QUEM SOMOS</Text>
        <Text style={{ fontSize: 24, fontWeight: "bold", color: NAVY, marginBottom: 8 }}>Quem Somos</Text>
        <Text style={{ fontSize: 9.5, color: GRAY_700, marginBottom: 16, lineHeight: 1.5 }}>Conheça a Alpha Condomínios e nossa missão de transformar a gestão condominial.</Text>
        <GoldDivider />
        ...
        <View style={{ backgroundColor: NAVY, borderRadius: 6, padding: 20 }}>
          <Text style={{ fontSize: 8, color: GOLD, letterSpacing: 2, fontWeight: "bold", marginBottom: 8 }}>NOSSA MISSÃO</Text>
...
function PageDiferenciais({ pg, total }: { pg: number; total: number }) {
  return (
    <Page size="A4" style={{ backgroundColor: WHITE }}>
      ...
      <View style={{ flex: 1, paddingHorizontal: 50, paddingTop: 30, paddingBottom: 52, justifyContent: "center" }}>
        <Text style={{ fontSize: 8, color: GOLD, letterSpacing: 2.5, fontWeight: "bold", marginBottom: 8 }}>POR QUE NOS ESCOLHER</Text>
        <Text style={{ fontSize: 24, fontWeight: "bold", color: NAVY, marginBottom: 8 }}>Nossos Diferenciais</Text>
        <Text style={{ fontSize: 9.5, color: GRAY_700, marginBottom: 16, lineHeight: 1.5 }}>O que torna a Alpha Condomínios diferente no mercado.</Text>
        <GoldDivider />
...
function ServicoCard({
  ...
  return (
    ...
    >
      ...
      <Text style={{ fontSize: 9.5, fontWeight: "bold", color: NAVY, marginBottom: 5 }}>{titulo}</Text>
...
function PageServicos({ pg, total }: { pg: number; total: number }) {
  const servicos = [
    { icon: <ISvcAdmin />,       titulo: "Administração de Condomínios", descricao: "Gestão completa de todas as atividades administrativas, financeiras e operacionais do condomínio, com foco em eficiência e transparência." },
    { icon: <ISvcSindico />,     titulo: "Síndico Profissional",         descricao: "Profissional qualificado e dedicado exclusivamente à gestão do condomínio, garantindo cumprimento de todas as obrigações legais. Inclui Seguro de Responsabilidade Civil (RC) de Síndico." },
    { icon: <ISvcCertificado />, titulo: "Certificado Digital",          descricao: "Emissão e gestão de certificados digitais para assinatura eletrônica de documentos, atas e contratos, garantindo validade jurídica e agilidade." },
    { icon: <ISvcSeguro />,      titulo: "Seguro Condominial",           descricao: "Contratação e gestão de apólices de seguro patrimonial, incêndio, responsabilidade civil e outros, com análise criteriosa de coberturas e custos." },
    { icon: <ISvcAVCB />,        titulo: "AVCB",                         descricao: "Assessoria completa para obtenção e renovação do Auto de Vistoria do Corpo de Bombeiros, garantindo conformidade legal e segurança dos moradores." },
    { icon: <ISvcJuridico />,    titulo: "Assessoria Jurídica",          descricao: "Suporte jurídico especializado em direito condominial, com orientação em assembleias, elaboração de documentos e resolução de conflitos." },
    { icon: <ISvcGarantidora />, titulo: "Garantidora de Crédito",       descricao: "Intermediação com empresas garantidoras para locação de unidades, facilitando a entrada de inquilinos e reduzindo inadimplência." },
    { icon: <ISvcOutros />,      titulo: "Dentre Outros",                descricao: "Soluções personalizadas conforme necessidades específicas de cada condomínio: manutenção predial, comunicação visual, automação, sustentabilidade e muito mais." },
  ];

  return (
    <Page size="A4" style={{ backgroundColor: WHITE }}>
      <PageHeader label="Serviços" />
      <View style={{ flex: 1, paddingHorizontal: 50, paddingTop: 22, paddingBottom: 52 }}>
        <Text style={{ fontSize: 8, color: GOLD, letterSpacing: 2.5, fontWeight: "bold", marginBottom: 6 }}>SOLUÇÕES ALPHA</Text>
        <Text style={{ fontSize: 20, fontWeight: "bold", color: NAVY, marginBottom: 6 }}>Nossos Serviços</Text>
        <GoldDivider />
        <Text style={{ fontSize: 9, color: GRAY_700, marginBottom: 16, lineHeight: 1.5 }}>Soluções completas para a administração do seu condomínio.</Text>
        <View style={{ flexDirection: "row", flexWrap: "wrap", justifyContent: "space-between" }}>
          {servicos.map((s) => (
            <ServicoCard key={s.titulo} icon={s.icon} titulo={s.titulo} descricao={s.descricao} />
          ))}
        </View>
      </View>
      <PageFooter current={pg} total={total} />
...
/* ================================================================
   PÁGINA 5 — PLANO ESSENCIAL
   ================================================================ */
function PagePlanoEssencial({
  pg, total, totalFmt, porUnidadeFmt,
}: {
...
/* ================================================================
   PÁGINA 6 — PLANO COMPLETO
   ================================================================ */
function PagePlanoCompleto({
  pg, total, totalFmt, porUnidadeFmt,
}: {
  pg: number; total: number; totalFmt: string; porUnidadeFmt: string;
}) {
...
/* ================================================================
   PÁGINA 7 — PLANO PREMIUM
   ================================================================ */
function PagePlanoPremium({
  pg, total, totalFmt, porUnidadeFmt,
}: {
  pg: number; total: number; totalFmt: string; porUnidadeFmt: string;
}) {
...
function PageComparativo({
  ...
  const linhas: { categoria?: string; item?: string; e: string; c: string; p: string }[] = [
    ...
    { item: "Cobrança Administrativa de Inadimplentes",        e: CHECK, c: CHECK, p: CHECK },
    { item: "Aplicativo Completo",                             e: CHECK, c: CHECK, p: CHECK },
    { item: "Rateio de Água e/ou Gás",                         e: DASH,  c: CHECK, p: CHECK },
    { item: "Planejamento Orçamentário Anual",                 e: DASH,  c: CHECK, p: CHECK },
    { item: "Gestão de Contas a Pagar",                        e: DASH,  c: CHECK, p: CHECK },
    { item: "Pagamento Online e Conciliação Bancária",         e: DASH,  c: CHECK, p: CHECK },
    { categoria: "OPERACIONAL" },
    { item: "Elaboração de Atas e Convocação de Assembleias",  e: DASH,  c: CHECK, p: CHECK },
    { item: "Relatórios Gerenciais",                           e: DASH,  c: CHECK, p: CHECK },
    { categoria: "PREMIUM" },
    { item: "Assessoria Jurídica Condominial",                 e: DASH,  c: DASH,  p: CHECK },
    { item: "Cumprimento das Obrigações Fiscais",              e: DASH,  c: DASH,  p: CHECK },
    { item: "Revisão da Convenção e Regimento Interno",        e: DASH,  c: DASH,  p: CHECK },
    { item: "Atendimento Prioritário SLA 12h",                 e: DASH,  c: DASH,  p: CHECK },
    { item: "Relatório Trimestral de Saúde Financeira",        e: DASH,  c: DASH,  p: CHECK },
    { item: "Suporte a Prazos de Manutenções e Contratos",     e: DASH,  c: DASH,  p: CHECK },
  ];
  ...
  return (
    <Page size="A4" style={{ backgroundColor: WHITE }}>
      ...
      <View style={{ flex: 1, paddingHorizontal: 50, paddingTop: 24, paddingBottom: 52 }}>
        ...
        <Text style={{ fontSize: 9, color: GRAY_700, marginBottom: 16, lineHeight: 1.5 }}>
